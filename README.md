...
|--------------|-------------------|
| 🔍 股票查询 / Stock Query | 输入代码获取数据、技术指标、趋势分析 |
| 📊 仪表盘 / Dashboard | 查看已存储的股票数据概览 |
| 📈 个股分析 / Stock Viewer | K线图、详细指标、风险评估 |
| 🔄 策略回测 / Backtest | 均线/RSI/MACD策略回测 |
| 🔎 股票筛选 / Screener | 多条件筛选已存储的股票 |
...
1. **首次使用数据库为空**，无默认股票数据
2. 在 **🔍 股票查询** 页面输入股票代码（如 159892）获取数据
3. 数据自动保存到本地数据库，下次查询时直接读取本地数据
4. 使用 **📊 仪表盘** 查看已存储的股票列表
5. 使用 **🔄 策略回测** 对特定股票进行策略测试
